...
# ─── 커밋/PR AI 트레일러 & 에이전트 패턴 ─────────────────────────────────────

# 커밋 메시지 / PR 본문에서 찾는 AI 서명 패턴
AI_COMMIT_TRAILERS = [
    r"Co-Authored-By:\s*.+",       # Co-authored-by 푸터
    r"Assisted-By:\s*.+",          # Assisted-by 푸터
    r"AI-assisted:\s*.+",          # AI-assisted 푸터
    r"Reviewed-By:\s*.+Copilot",   # Copilot 리뷰
]

# 커밋/PR 푸터에서 찾는 도구 고정 문구
AI_TOOL_FOOTERS = [
    r"Generated with \w+",
    r"Generated by [Aa]I",
    r"AI-generated",
...
KNOWN_AGENT_AUTHORS = {
    ...
    "claude[bot]":                  "Claude",
    "anthropic[bot]":               "Anthropic Claude",
    "openai[bot]":                  "OpenAI",
    "codex[bot]":                   "OpenAI Codex",
    "cursor[bot]":                  "Cursor",
    "cody[bot]":                    "Sourcegraph Cody",
    "amzn-bot[bot]":                "Amazon Q",
    "codeium[bot]":                 "Codeium",
    "sweep[bot]":                   "Sweep AI",
    "aider[bot]":                   "Aider",
    "devin[bot]":                   "Devin",
...
AI_BRANCH_PATTERNS = {
    ...
    r"^codex/":                 "OpenAI Codex",
    r"^claude/":                "Claude",
    r"^cursor/":                "Cursor",
    r"^cody/":                  "Sourcegraph Cody",
    r"^ai/":                    "AI (general)",
    r"^copilot-":               "GitHub Copilot",
    r"^codex-":                 "OpenAI Codex",
    r"^claude-":                "Claude",
...
# AI 지시 파일 내부에서 찾는 제어 키워드
AI_INSTRUCTION_DIRECTIVES = [
    r"(?i)never",
    r"(?i)do\s+not",
    r"(?i)don't",
    r"(?i)always",
...
# AI 지시 파일 내부에서 찾는 실행 명령 키워드
AI_INSTRUCTION_COMMANDS = [
    r"(?i)run\s+this",
    r"(?i)execute",
    r"(?i)build",
    r"(?i)test",
...
class AIUtilizationEvaluator:
    ...
    def evaluate(
        ...
        # 9. 커밋/PR AI 트레일러 (Co-authored-by, Assisted-by 등)
        items.append(self._score_commit_ai_trailers(commits, pulls))

        # 10. 에이전트 작성자 (copilot-swe-agent[bot] 등)
        items.append(self._score_agent_authors(commits, pulls))

        # 11. AI 에이전트 지시 파일 (AGENTS.md, CLAUDE.md 등 + 내용 분석)
        items.append(self._score_ai_instruction_files(tree))

        # 12. AI 에이전트 브랜치 (copilot/, claude/, codex/ 등)
        items.append(self._score_agent_branches(branches))
    ...
    def _score_commit_ai_trailers(self, commits: list, pulls: list) -> AIScore:
        """9. 커밋/PR AI 트레일러 평가"""
        score = 0.0
        evidence = []
        trailer_hits = 0
        ...
        for c in commits:
            ...
            for pattern in AI_COMMIT_TRAILERS:
                if re.search(pattern, msg, re.IGNORECASE):
                    trailer_hits += 1
                    evidence.append(f"커밋 트레일러: {c.get('sha', '')[:8]} → {pattern}")
                    break
            for pattern in AI_TOOL_FOOTERS:
                if re.search(pattern, msg, re.IGNORECASE):
                    footer_hits += 1
                    evidence.append(f"커밋 도구 문구: {c.get('sha', '')[:8]} → {pattern}")
                    break
        ...
        for p in pulls:
            ...
            text = title + "\n" + body
            for pattern in AI_COMMIT_TRAILERS + AI_TOOL_FOOTERS:
                if re.search(pattern, text, re.IGNORECASE):
                    pr_trailer_hits += 1
                    evidence.append(f"PR AI 서명: #{p.get('number', '?')} → {pattern}")
                    break

        total_hits = trailer_hits + footer_hits + pr_trailer_hits
        if total_hits > 0:
            score = min(10, total_hits * 2.0)
            details = (
                f"커밋 트레일러 {trailer_hits}건, 도구 문구 {footer_hits}건, "
                f"PR AI 서명 {pr_trailer_hits}건 감지"
            )
        else:
            details = "커밋/PR에서 AI 트레일러 미감지"
    ...
    def _score_agent_authors(self, commits: list, pulls: list) -> AIScore:
        """10. 에이전트 작성자 평가"""
        score = 0.0
        evidence = []
        agent_commits = 0
        ...
        for c in commits:
            ...
            # 이름 매칭
            if name in KNOWN_AGENT_AUTHORS:
                agent_commits += 1
                found_agents.add(KNOWN_AGENT_AUTHORS[name])
                evidence.append(f"에이전트 커밋: {name} ({KNOWN_AGENT_AUTHORS[name]})")
                continue

            # 이메일 매칭
            email_local = email.split("@")[0].lower() if email else ""
            email_domain = email.split("@")[1].split(".")[0].lower() if "@" in email else ""
            for agent_key, agent_name in KNOWN_AGENT_EMAILS.items():
                if agent_key in email_local or agent_key in email_domain:
                    agent_commits += 1
                    found_agents.add(agent_name)
                    evidence.append(f"에이전트 커밋 이메일: {email} ({agent_name})")
                    break
        ...
        for p in pulls:
            ...
            if login in KNOWN_AGENT_AUTHORS:
                agent_prs += 1
                found_agents.add(KNOWN_AGENT_AUTHORS[login])
                evidence.append(f"에이전트 PR: #{p.get('number', '?')} by {login} ({KNOWN_AGENT_AUTHORS[login]})")

        total = agent_commits + agent_prs
        if total > 0:
            score = min(10, total * 2.0)
            if len(found_agents) >= 3:
                score = min(10, score + 1.0)
            details = (
                f"에이전트 커밋 {agent_commits}건, PR {agent_prs}건 감지. "
                f"에이전트 종류: {', '.join(sorted(found_agents))}"
            )
        else:
            details = "에이전트 작성자 미감지 (알려진 봇/에이전트 아님)"
    ...
    def _score_ai_instruction_files(self, tree: list) -> AIScore:
        """11. AI 에이전트 지시 파일 평가"""
        file_paths = [item.get("path", "") for item in tree if item.get("type") == "blob"]
        found_files = {}

        for path in file_paths:
            for pattern, tool in AI_INSTRUCTION_FILES.items():
                if path == pattern or path.endswith("/" + pattern):
                    found_files[path] = tool
                    break
        ...
        for file_path, tool in found_files.items():
            # 파일 내용 가져와서 지시사항/실행명령 분석
            content = self._fetch_file_content(file_path)
            if content:
                for pattern in AI_INSTRUCTION_DIRECTIVES:
                    matches = re.findall(pattern, content)
                    directive_count += len(matches)
                for pattern in AI_INSTRUCTION_COMMANDS:
                    matches = re.findall(pattern, content)
                    command_count += len(matches)
                evidence.append(f"AI 지시 파일: {file_path} [{tool}] (지시 {directive_count}건, 명령 {command_count}건)")
            else:
                evidence.append(f"AI 지시 파일: {file_path} [{tool}] (내용 분석 불가)")

        # 파일 존재 점수
        if found_files:
            score = min(10, len(found_files) * 3.0)
            # 지시사항 포함 보너스
            ...
            if command_count > 0:
                score = min(10, score + 1.0)
            details = (
                f"AI 지시 파일 {len(found_files)}개 감지. "
                f"지시사항 {directive_count}건, 실행 명령 {command_count}건"
            )
        else:
            details = "AGENTS.md, CLAUDE.md 등 AI 지시 파일 미감지"
    ...
    def _score_agent_branches(self, branches: list) -> AIScore:
        """12. AI 에이전트 브랜치 평가"""
        score = 0.0
        evidence = []
        found_branches = {}
        ...
        for branch in branches:
            branch_name = branch.get("name", "") if isinstance(branch, dict) else str(branch)
            for pattern, tool in AI_BRANCH_PATTERNS.items():
                if re.search(pattern, branch_name):
                    found_branches[branch_name] = tool
                    evidence.append(f"AI 브랜치: {branch_name} [{tool}]")
                    break

        if found_branches:
            score = min(10, len(found_branches) * 2.5)
            tools = set(found_branches.values())
            if len(tools) >= 2:
                score = min(10, score + 1.0)
            details = (
                f"AI 에이전트 브랜치 {len(found_branches)}개 감지. "
                f"도구: {', '.join(sorted(tools))}"
            )
        else:
            details = "AI 에이전트 브랜치 미감지 (copilot/, claude/, codex/ 등)"
...
def integrate_ai_evaluation(evaluator_instance):
    ...
    def enhanced_evaluate() -> dict:
        ...
        commits = evaluator_instance.fetch_commits()
        pulls = evaluator_instance.fetch_pulls()
        branches = evaluator_instance.fetch_branches()
