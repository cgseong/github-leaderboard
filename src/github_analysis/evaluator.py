...
class RepositoryEvaluator:
    ...
    def evaluate(self, owner: str, repo: str, repo_info: dict[str, Any] | None = None) -> EvaluationReport:
        warnings: list[str] = []
        ...
        pulls_all = self._safe_call(lambda: self.client.get_pulls(owner, repo, state="all"), warnings, "All PRs")
        ...
        for pr in pulls_all:
            ...
            if user:
                if user not in contributor_stats:
                    contributor_stats[user] = {"login": user, "C": 0, "P_open": 0, "P_merged": 0, "R_total": 0, "R_approve": 0, "R_changes": 0}
                if pr.get("state") == "open":
                    contributor_stats[user]["P_open"] += 1
                elif pr.get("merged_at"):
                    contributor_stats[user]["P_merged"] += 1

            pr_number = pr.get("number")
            if pr_number:
                reviews = self._safe_call(lambda: self.client.get_pull_reviews(owner, repo, pr_number), warnings, f"PR #{pr_number} Reviews")
                for review in reviews:
                    reviewer = review.get("user", {}).get("login")
                    if not reviewer:
                        continue
                    if reviewer not in contributor_stats:
                        contributor_stats[reviewer] = {"login": reviewer, "C": 0, "P_open": 0, "P_merged": 0, "R_total": 0, "R_approve": 0, "R_changes": 0}
                    
                    contributor_stats[reviewer]["R_total"] += 1
                    state = review.get("state", "").upper()
                    if state == "APPROVED":
                        contributor_stats[reviewer]["R_approve"] += 1
                    elif state == "CHANGES_REQUESTED":
                        contributor_stats[reviewer]["R_changes"] += 1

        contributor_details = list(contributor_stats.values())
        contributor_details.sort(key=lambda x: (x["C"], x["P_merged"]), reverse=True)
